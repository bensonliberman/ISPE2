...
def get_training_attendee_iqa(token, offset):
    ...
    url = f"https://www2.ispe.org/Asi.Scheduler_IMIS/api/iqa?QueryName=$/ISPEIQA/Queries/HubSpot/HS_TrainingAttendees&offset={offset}&limit=100"
...
# get_training_attendee_iqa(authorization(),0)
...
def create_hs_deal(imis_id, total_charges, event_name, registrant_code, purchase_date_mil, last_updated_mil, sole_num, function_title):
    today = date.today()
    # open logging file
    ...
    payload = {
        ...
        "properties": [
            {
                "name": "dealname",
                "value": f"Training_Attendee_{event_name}"
            },
            {
                "name": "dealstage",
                "value": "770859"
            ...
            {
                "name": "training_attendees_sole_num",
                "value": sole_num
            },
            {
                "name": "event_name",
...
def main_function():
    training_attendee_data = {}  # initializes a dictionary that will store the contact data

    has_next = True
    offset = 0
    while has_next:
        data = get_training_attendee_iqa(authorization(), offset)  # gets a new request for the new offset range
        # for page in data:
        for i in data['Items']['$values']:
            for value in i['Properties']['$values']:  # for each contact in data
                if value['Name'] == 'ResultRow':  # removes the ResultRow value since that is not needed in HubSpot
                    ...
                    try:
                        ...
                        training_attendee_data[value['Name']] = value['Value']
                    except:
                        training_attendee_data[value['Name']] = ''
            # print(training_attendee_data)  # prints training_attendee_data dictionary for testing

            # define fields and convert date fields to unix milliseconds
            st_id = training_attendee_data['ST_ID']
            meeting = training_attendee_data['Meeting']
            function_title = training_attendee_data['Function_Title']
            order_date = training_attendee_data['Order_Date']
            # print(order_date)
            order_date_mil = int(datetime.strptime(order_date, '%Y-%m-%dT%H:%M:%S').timestamp() * 1000)
            registrant_class = training_attendee_data['Registrant_Class']
            total_charges = training_attendee_data['Total_Charges']['$value']
            last_updated = training_attendee_data['Last_Updated'].split(".")[0]
            last_updated_mil = int(datetime.strptime(last_updated, '%Y-%m-%dT%H:%M:%S').timestamp() * 1000)
            sole_num = training_attendee_data['Sole_Num']
            ...
            if cur_date - datetime.strptime(order_date, "%Y-%m-%dT%H:%M:%S") <= timedelta(days=2*365):
                ...
                try:
                    ...
                    training_attendee_data = {}  # clears training_attendee_data dictionary for next item in request
                    continue
                ...
                assoc_deals_training_attendee_sole_nums = []

                if not assoc_deals:
                    create_hs_deal(
                        imis_id=st_id, total_charges=total_charges, registrant_code=registrant_class,
                        purchase_date_mil=order_date_mil, last_updated_mil=last_updated_mil, event_name=meeting,
                        sole_num=sole_num, function_title=function_title
                    )
                ...
                for deal_id in assoc_deals:
                    deal_url = f'https://api.hubapi.com/deals/v1/deal/{deal_id}?hapikey={hs_api_key}&property=training_attendee_sole_num'
                    headers = {"Content-Type": "application/json"}
                    ...
                    try:
                        deal_sole_num = res['properties']['training_attendee_sole_num']['value']
                    except:
                        deal_sole_num = ''

                    assoc_deals_training_attendee_sole_nums.append(deal_sole_num)

                # if HS Deal cart ID equal to iMIS cart ID, deal already created; continue
                if sole_num in assoc_deals_training_attendee_sole_nums:
            ...
            training_attendee_data = {}  # clears the training_attendee_data dictionary for the next contact in the request
